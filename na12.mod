...
NEURON {
	...
	USEION lca WRITE ilca VALENCE 2
	RANGE m, h, gna, gbar,ina, ica, ilca
	GLOBAL tha, thi1, thi2, qa, qi, qinf, thinf,cana
	RANGE minf, hinf, mtau, htau 
	GLOBAL Ra, Rb, Rd, Rg
	GLOBAL q10, temp, tadj, vmin, vmax, vshift,hw, hc, bw
}

PARAMETER {
	cana=0.4
	gbar = 1000   	(pS/um2)	: 0.12 mho/cm2
	vshift = -4	(mV)		: voltage shift (affects all)
								
	tha  = -44	(mV)		: v 1/2 for act		
	qa   = 6.8	(mV)		: act slope	
...
BREAKPOINT {
        SOLVE states
        gna = tadj*gbar*m*m*m*h*bw+tadj*gbar*m*m*m*hw*h2
	ina = (1e-4) * gna * (v - ena)
